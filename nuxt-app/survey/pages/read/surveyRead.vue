                    ...
                    <v-col>
                        <v-card outlined class="survey-card">
                            ...
                            <v-radio-group v-model="answers[index]" :name="`question-${index}`">
                                <v-radio v-for="(option, idx) in survey.answers[index]" :key="option" :label="option"
                                    :value="idx">
...
export default defineComponent({
    setup() {
        ...
        async function submitSurvey() {
            const allAnswered = surveyStore.survey.questions.every((_, index) => answers.value[index] !== undefined)
            ...
            const payload = { surveyId: route.params.id, answers: answers.value }
            await surveyStore.requestCreateAnswerToDjango(payload)
            await router.push('/survey/thanks')
        }

        onMounted(async () => {
            await surveyStore.requestSurveyToDjango(route.params.id)
        })
