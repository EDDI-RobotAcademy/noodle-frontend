...
export const meetingRecorderActions = {
	async requestGenerateMeetingRecordingSummary(payload): Promise<boolean> {
		...
		try {
			...
			console.error(
				"requestGenerateMeetingSummary() 중 에러 발생:",
				error
			);
			return false;
		}
	...
	async requestGetMeetingRecordingSummary(userToken: string): Promise<any> {
		...
		try {
			...
			for (let attempt = 0; attempt < maxAttempts; attempt++) {
				response = await djangoAxiosInst.post(
					"/get-meeting-recording-summary/get",
					{ userToken }
				);
			...
			console.error(
				"requestGetMeetingRecordingSummary() 중 문제 발생:",
				error
			);
		}
	},
