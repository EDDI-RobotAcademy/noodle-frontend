      ...
      <div class="leftbox_title">
        ...
        <div class="switch white">
          ...
          <input
            ...
            checked
          />
          <label for="switch-off">states</label>
...
.leftbox_title {
  ...
  justify-content: space-between; /* 왼쪽과 오른쪽에 콘텐츠를 배치 */
...
.switch {
  ...
  height: var(--switch-height, 50px); /* 스위치의 높이를 50px로 설정 (변경 가능하도록 변수 사용) */
  position: relative; /* 자식 요소의 위치를 기준으로 상대적으로 위치 설정 */
  width: var(--switch-width, 100px); /* 스위치의 너비를 100px로 설정 (변경 가능하도록 변수 사용) */
  margin-left: auto; /* 스위치를 오른쪽으로 정렬 */
}

/* 스위치 글자 관련 설정 */
.switch label {
  color: #fff; /* 라벨의 텍스트 색상을 흰색으로 설정 */
  font-size: 15px; /* 라벨의 폰트 크기를 20px로 설정 */
  font-weight: 500; /* 라벨의 폰트 두께를 300으로 설정 (얇은 텍스트) */
  line-height: var(--switch-height, 50px); /* 글의 세로 정렬을 위해 line-height를 스위치 높이에 맞춤 */
  text-transform: uppercase; /* 라벨 텍스트를 대문자로 변환 */
  transition: color 0.2s ease; /* 라벨의 색상 변화에 0.2초의 전환 효과 적용 */
  width: 70px; /* 라벨의 너비를 100px로 설정 */
}

.switch label:nth-of-type(1) {
  left: -75%; /* 첫 번째 라벨을 스위치 왼쪽에 위치시키기 위해 왼쪽으로 75% 이동 */
...
.switch label:nth-of-type(2) {
  ...
  right: -75%; /* 두 번째 라벨을 스위치 오른쪽에 위치시키기 위해 오른쪽으로 75% 이동 */
  text-align: left; /* 두 번째 라벨의 텍스트를 왼쪽 정렬 */
}

.switch input {
  height: var(--switch-height, 50px); /* 스위치 입력의 높이를 50px로 설정 */
  left: 0; /* 입력 요소를 왼쪽에 위치 */
  opacity: 0; /* 입력 요소를 보이지 않도록 투명도 0으로 설정 */
  position: absolute; /* 입력 요소의 위치를 절대 위치로 설정 */
  top: 0; /* 입력 요소를 상단에 위치 */
  width: var(--switch-width, 120px); /* 스위치 입력의 너비를 120px로 설정 */
  z-index: 2; /* 입력 요소를 다른 요소보다 앞에 표시 (레이어 순서 설정) */
}

.switch input:checked ~ label:nth-of-type(1) {
  color: #fff; /* 첫 번째 라벨의 텍스트 색상을 흰색으로 설정 (스위치가 선택되었을 때) */
}

.switch input:checked ~ label:nth-of-type(2) {
  color: #808080; /* 두 번째 라벨의 텍스트 색상을 회색으로 설정 (스위치가 선택되었을 때) */
}

.switch input ~ :checked ~ label:nth-of-type(1) {
  color: #808080; /* 첫 번째 라벨의 텍스트 색상을 회색으로 설정 (스위치가 선택되지 않았을 때) */
}

.switch input ~ :checked ~ label:nth-of-type(2) {
  color: #fff; /* 두 번째 라벨의 텍스트 색상을 흰색으로 설정 (스위치가 선택되지 않았을 때) */
}

.switch input:checked ~ .toggle {
  left: 4px; /* 스위치가 선택되었을 때 토글이 왼쪽에 위치 */
}

.switch input ~ :checked ~ .toggle {
  left: 100px; /* 스위치가 선택되지 않았을 때 토글이 오른쪽에 위치 */
}

.switch input:checked {
  z-index: 0; /* 입력 요소의 z-index를 0으로 설정하여 다른 요소보다 뒤로 감 */
}

.toggle {
  background: #4a4a4a; /* 토글의 배경색을 어두운 회색으로 설정 */
  border-radius: 50%; /* 토글의 모양을 원형으로 설정 (50% 반경) */
  height: calc(var(--switch-height, 50px) - 8px); /* 토글의 높이를 스위치 높이보다 약간 작게 설정 */
  left: 0; /* 토글을 왼쪽에 위치 */
  position: absolute; /* 토글의 위치를 절대 위치로 설정 */
  top: 4px; /* 토글을 위에서 4px 내려서 위치 */
  transition: left 0.2s ease; /* 토글이 이동할 때 0.2초의 전환 효과 적용 */
  width: calc(var(--switch-height, 50px) - 8px); /* 토글의 너비를 스위치 높이와 비슷하게 설정 */
  z-index: 1; /* 토글을 다른 요소보다 앞에 표시 */
}

.toggle.checked {
  left: calc(var(--switch-width, 120px) - calc(var(--switch-height, 50px) - 8px)); /* 스위치가 선택되었을 때 토글이 오른쪽에 위치 */
}
