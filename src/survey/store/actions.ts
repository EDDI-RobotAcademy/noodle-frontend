...
export type SurveyActions = {
	requestSurveyToDjango(
		context: ActionContext<SurveyState, any>,
		surveyId: number
	): Promise<void>;
	requestCreateSurveyToDjango(
		context: ActionContext<SurveyState, any>,
		payload: {
			surveyId: number;
			questions: [string];
	...
	requestCreateAnswerSurveyToDjango(
		context: ActionContext<SurveyState, any>,
		payload: {
			surveyId: number;
			answer: [string];
	...
	requestSurveyListToDjango(
		context: ActionContext<SurveyState, any>
	): Promise<number[]>;
};

const actions: SurveyActions = {
	async requestSurveyToDjango(
		context: ActionContext<SurveyState, any>,
		surveyId: number
	): Promise<void> {
		try {
			const res: AxiosResponse<Survey> =
				await axiosInst.djangoAxiosInst.get(`/survey/read/${surveyId}`);
	...
	async requestCreateSurveyToDjango(
		context: ActionContext<SurveyState, any>,
		payload: {
			surveyId: number;
			questions: [string];
	...
	async requestCreateAnswerSurveyToDjango(
		context: ActionContext<SurveyState, any>,
		payload: {
			surveyId: number;
			answer: [string];
	...
	async requestCreateAnswerToDjango(
		context: ActionContext<SurveyState, any>,
		payload: {
			surveyId: number;
			answers: number[];
		...
		try {
			const res: AxiosResponse = await axiosInst.djangoAxiosInst.post(
				"/survey/save",
				{ surveyId: surveyId, answer: answer }
			);
	...
	async requestSurveyListToDjango(
		...
		try {
			const res: AxiosResponse<any, any> =
				await axiosInst.djangoAxiosInst.get("/survey/list");
